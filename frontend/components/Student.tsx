...
const User = () => {
  ...
  //   try {
  //     let student = fetch(`http://localhost:3000/api/student/${email}`, {
  //       headers: {
  //         "Content-Type": "application/json",
  //       },
  //       method: "GET",
  //     }).then(() => {
  //       // setStudent(JSON.parse(localStorage.getItem("currentUser")));
  //       console.log(student);
  //     });
  //   } catch (error) {
  //     console.log(error);
        ...
        <div className="absolute left-[72px] top-[72px] px-60 p-32 h-[4000px] space-y-4">
          ...
          <div>
            <input
              name="avatar_url"
              type="text"
              placeholder={user?.avatar_url ? user?.avatar_url : "Image URL"}
            />
            <button className="border-4" onClick={(e) => updateStudent(e)}>
              Ažuriraj
            </button>
          </div>
          <div>
            <button className="border-4" onClick={(e) => deleteStudent(e)}>
              Izbriši studenta
            </button>
          </div>
